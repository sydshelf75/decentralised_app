    ...
    <div className="sticky top-0 z-10 px-6 w-full bg-gray-900 border-b border-gray-800">
      <div className="flex items-center justify-between gap-6 w-full h-16">
        ...
        <div className="relative flex-1 max-w-2xl">
          ...
          <Input className="w-full bg-gray-800 border-gray-700 rounded-full py-3 pl-12 pr-20 text-gray-200 placeholder-gray-400 focus:ring-indigo-500 text-base" placeholder="Ask anything... AI + Human experts will answer" value={question} onChange={(e) => setQuestion(e.target.value)} onKeyDown={(e) => e.key === "Enter" && handleAsk()} />
          <Button disabled={loading} onClick={handleAsk} className="absolute right-2 top-1/2 -translate-y-1/2 bg-indigo-600 hover:bg-indigo-500 rounded-full px-5">
            {loading ? "..." : "Ask"}
          </Button>
        </div>
        ...
        {session ? (
          ...
          <Link href="/login">
            <Button className="h-10 text-base font-semibold bg-indigo-600 hover:bg-indigo-500 rounded-md cursor-pointer">Log In</Button>
          </Link>
        )}
      </div>
    </div>
